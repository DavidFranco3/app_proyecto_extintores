...
import '../EncuestasDatos/encuestas_datos.dart';

class CrearEncuestaDatosScreen extends StatefulWidget {
  ...
  @override
  CrearEncuestaDatosScreen(
      {required this.showModal,
      required this.onCompleted,
      required this.accion,
      required this.data});

  _CrearEncuestaDatosScreenState createState() => _CrearEncuestaDatosScreenState();
}

class _CrearEncuestaDatosScreenState extends State<CrearEncuestaDatosScreen> {
  ...
  void _guardarEncuesta(Map<String, dynamic> data) async {
    ...
    try {
      ...
      var response =
          await encuestaDatosInspeccionService.registraEncuestaDatosInspeccion(dataTemp);
      ...
      if (response['status'] == 200) {
        ...
        LogsInformativos(
            "Se ha registrado la encuesta de datos ${data['nombre']} correctamente",
            dataTemp);
        _showDialog(
            "Encuesta de datos agregada correctamente", Icons.check, Colors.green);
      } else {
  ...
  void _editarEncuesta(String id, Map<String, dynamic> data) async {
    ...
    try {
      final encuestaDatosInspeccionService = EncuestaDatosInspeccionService();
      var response = await encuestaDatosInspeccionService
          .actualizarEncuestaDatosInspeccion(id, dataTemp);
      // Verifica el statusCode correctamente, según cómo esté estructurada la respuesta
      if (response['status'] == 200) {
        // Asumiendo que 'response' es un Map que contiene el código de estado
        setState(() {
        ...
        LogsInformativos(
            "Se ha actualizado la encuesta de datos ${data['nombre']} correctamente",
            dataTemp);
        _showDialog(
            "Encuesta de datos actualizada correctamente", Icons.check, Colors.green);
      } else {
        // Maneja el caso en que el statusCode no sea 200
        setState(() {
          _isLoading = false;
  ...
  void returnPrincipalPage() {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => EncuestasDatosPage()),
    ).then((_) {
